...
class FusePlugin @Inject constructor(
     ...
     */
    fun toggleMealMarker(
        now: Long,
        ohneVorschuss: Boolean = false,
        /**
         * Kennung des BEDIENEREIGNISSES. Ein wiederholter oder verspaeteter
         * Dialog-Rueckruf traegt dieselbe - und darf den Zustand dann NICHT
         * ein zweites Mal umschalten. Ohne Kennung gilt das bisherige
        ...
        if (armed) {
            ...
            //
            // ZUERST der Ledger, DANN die Preferences. Endet der Prozess
            // dazwischen, steht der Widerruf fest und der Marker gilt beim
            // naechsten Lesen als zurueckgenommen. Umgekehrt waere die
            // Autorisierung aus den Preferences verschwunden, ohne dass
            // irgendwo stuende, dass sie widerrufen wurde - und der
            // naechste Druck haette keine Marke zu verbrauchen.
            //
            // Der Widerruf beendet die AUTORISIERUNG. Er storniert nichts:
            // bereits abgegebenes Insulin, offene Pumpenauftraege,
            // Transportreservierungen und `primeSpentU` bleiben stehen.
        ...
        //
        // Kennung und Verbrauch der Widerrufsmarke werden GEMEINSAM
        // festgeschrieben. Wuerde erst bewaffnet und spaeter die Marke
        // zugeordnet, koennte ein Absturz dazwischen eine zweite volle
        // Huelle erlauben.
        //
        // Auch hier Ledger vor Preferences: endet der Prozess dazwischen,
        // ist die Marke verbraucht, aber kein Marker aktiv - der naechste
...
override fun fuseMarkerArmed(now: Long): Boolean = mealMarkerActive(now)
    ...
    override fun configuration(): JSONObject =
        JSONObject()
            ...
            .put(FuseBooleanKey.LivenessReboundEvidenceExceptionEnabled, preferences)
            .put(FuseBooleanKey.LivenessBookingExitWithoutReArmEnabled, preferences)
            .put(FuseBooleanKey.LivenessDriveHoldEnabled, preferences)
            .put(FuseIntKey.EarlyAdaptiveMealHorizonMin, preferences)
            ...
            .put(FuseIntKey.ZeroLatchReasonGoneExitCycles, preferences)
            .put(FuseDoubleKey.CorrectionSeriesCapU, preferences)
            .put(FuseIntKey.CorrectionSeriesWindowMin, preferences)
            .put(FuseBooleanKey.PartialRecoveryEnabled, preferences)
    ...
    override fun applyConfiguration(configuration: JSONObject) {
        configuration
            ...
            .store(FuseBooleanKey.LivenessReboundEvidenceExceptionEnabled, preferences)
            .store(FuseBooleanKey.LivenessBookingExitWithoutReArmEnabled, preferences)
            .store(FuseBooleanKey.LivenessDriveHoldEnabled, preferences)
            .store(FuseIntKey.EarlyAdaptiveMealHorizonMin, preferences)
            ...
            .store(FuseIntKey.ZeroLatchReasonGoneExitCycles, preferences)
            .store(FuseDoubleKey.CorrectionSeriesCapU, preferences)
            .store(FuseIntKey.CorrectionSeriesWindowMin, preferences)
            .store(FuseBooleanKey.PartialRecoveryEnabled, preferences)
     ...
     * danach diesen Haken jedes Plugins. Ein an der Preference gesetzter
     * Zusammenfassungs-Provider laesst genau diese Zuweisung mit
     * IllegalStateException scheitern - der FUSE-Bildschirm stuerzte beim
     * Oeffnen ab, der Loop lief weiter. Hier wird aus dem Eintrag Zustand plus
     * Erklaerung, auch fuer einen unzulaessigen Altwert ("wirkt als AUS").
     */
    override fun updatePreferenceSummary(pref: Preference) {
        super.updatePreferenceSummary(pref)
        if (pref.key == FuseIntKey.EarlyAdaptiveMealHorizonMin.key) {
            pref.summary = EarlyAdaptiveMealHorizonText.zusammenfassung(
                preferences.get(FuseIntKey.EarlyAdaptiveMealHorizonMin),
                rh.gs(R.string.fuse_early_adaptive_meal_horizon_summary),
            )
        }
    }
    ...
    override fun addPreferenceScreen(preferenceManager: PreferenceManager, parent: PreferenceScreen, context: Context, requiredKey: String?) {
        ...
        cat("fuse_dosing", "Dosierung und Grenzen") {
            ...
            // (Review 18.09.: eine 7 waere still AUS gewesen). KEINE dialogMessage:
