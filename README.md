...
- **One control: a meal button.** It declares that a meal is starting — not how big it is. It creates no carb entry and no COB. Pressing it opens a three-way dialog (see below), and it pins the **MEAL dosing profile** for a configurable authorization period (default 120 min): while it lasts, the larger meal exposure limit applies; when it expires, the cycle falls back to CORRECTION on its own.
...
| **dosing profile** | Every cycle runs in exactly one of two profiles: **CORRECTION** — the ordinary state — or **MEAL**, which exists only while a marker press has pinned a meal authorization (default 120 min). The profile decides how much exposure FUSE may finance. |
| **exposure limit** | A per-profile ceiling on FUSE-financed insulin exposure. A binding final check certifies the finished amount of every cycle against `min(iobTH, maxIOB, profile limit)` minus what is already occupied. |
| **liveness channel** | A bounded follow-up channel that makes demand the main path has already recognised deliverable when the guard and tail checks starve it — never by addition, always as `max(normal, liveness)`, under the profile's exposure limit. Switchable, ships off. |
...
3. **Which limit binds?** — the final amount is checked against three kinds of limit: what the *curve* allows (the guard floor, and the longer-horizon tail check that also counts insulin still to act); what the *insulin budget* allows (the per-cycle SMB cap, the iobTH/maxIOB headroom, the active dosing profile's exposure limit, and insulin already handed over but not yet visible); and what the *situation* allows (pump increment, meal-episode allowance, deadbands, health gates).
...
#### The two dosing profiles

Since 29.08.2026 every cycle is assigned exactly one **dosing profile** before any amount is computed. **CORRECTION** is the ordinary state. **MEAL** exists only while a marker press has pinned a meal authorization, for the period frozen at the press (default 120 min); when it ends, the very next cycle is CORRECTION again — there is no mode switch and nothing to remember to turn off. A rising glucose, a high drive, or a still-living evidence episode deliberately do *not* reach MEAL: only the explicit human press does.

The profile decides two numbers. First, the **exposure limit**: a binding final check certifies the finished amount of every cycle — whatever stage produced it — against `min(iobTH, maxIOB, profile exposure limit)` minus the exposure already occupied (capIOB plus open transport liability). It is a pure amount check that deliberately does not re-run the curve checks, it rounds down to the pump raster, and when it zeroes a positive request the block is named `EXPOSURE` — it never creates a protective zero on its own. Second, a per-profile **cap on the demand ratio**, the fraction of computed demand a single cycle may express. The measured motivation: a correction burst on 27.08.2026 delivered 2.5 U in twelve minutes because no context-dependent ceiling existed in the normal path while iobTH and maxIOB still left 4–6 U of room.

The resulting SMB status is exported **typed** every cycle — `FREE`, `STOP` with a typed stop reason (`EXPOSURE`, `GUARD`, `TAIL`, `LEDGER`, `PUMP`, …), or `NO_DEMAND`, which is deliberately not an alarm state — together with the requested, capped and published amounts (published = handed into the AAPS result, not pump-confirmed) and the typed source of the final amount. No display guesses the state from reason texts. The four profile values ship with a working start set (CORRECTION 3.0 U / MEAL 7.0 U exposure, ratio caps 0.20 / 0.35); they are runtime defaults and tuning knobs, not therapy recommendations, and an update never overwrites a value the user has set.
...
Since 22.08.2026 the policy also carries the **liveness channel**, switchable and off by default. Its measured motivation: at high glucose after meals, the carbohydrate-free pessimistic curve and the tail check rationed or zeroed every delivery for well over an hour while the main path itself kept recognising demand. The channel does not invent demand — it makes that already-recognised demand deliverable, capped by its own ratio, by the active profile's exposure limit, by iobTH/maxIOB, by transport liability and by the pump raster, and merged as `max(normal, liveness)`, never as an addition. Measured lows, measured falling, rebound windows, signal faults, ledger holds and the pump gate remain absolute for it, and any manual intervention disarms it with a restart-safe re-arm lock.
...
| Central dosing profiles (CORRECTION/MEAL), binding exposure check, typed SMB status | Implemented and tested | New (29.08.2026), first field data |
| Liveness channel (bounded follow-up dosing) | Implemented and tested; switchable, ships off | Operator-enabled since 22.08.2026 |
...
| Correction vs. meal dosing room | One global budget shaped by factor rules | Two explicit dosing profiles with per-profile exposure ceilings and demand-ratio caps, certified on the final amount |
...
- `3.4.2.5+fuse1.0.0-toni` is the production line: it is what loops on the real pump. The earlier line is frozen under the tag `fuse-0.1.0-beta`; its branch has been removed, because its history is fully contained in the production line.
