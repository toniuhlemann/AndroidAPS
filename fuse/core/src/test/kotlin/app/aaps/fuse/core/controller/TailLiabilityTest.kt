...
class TailLiabilityTest {
    ...
    fun `erschoepfter Schwanz blockt - aber ohne Zero-Temp`() {
        ...
        val d = FuseController.decide(state(), pred(250.0), tail = tail, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `verbleibender Schwanzspielraum deckelt die Menge und wird beziffert`() {
        ...
        val d = FuseController.decide(state(), pred(400.0), tail = tail, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `ein unbrauchbarer Bericht greift nicht in die Entscheidung ein`() {
        ...
        val d = FuseController.decide(state(), pred(400.0), tail = kaputt, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `ohne Schwanz-Guard bleibt die Entscheidung unveraendert`() {
        val mit = FuseController.decide(state(), pred(400.0), tail = null, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
        assertNull(mit.tail)
    ...
    fun `der Nahzonen-Guard hat Vorrang vor dem Schwanz`() {
        ...
        val d = FuseController.decide(state(), pred(250.0, minLower = 60.0), tail = tail, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
