...
class FuseCycleRunner(
    ...
    data class Outcome(
        ...
        /** Wieviel die Klemmung DIESES Zyklus weggenommen hat [U]. */
        val deferredClampReductionU: Double = 0.0,
        /** Wann geklemmt wurde; 0 = in diesem Zyklus nicht geklemmt. */
        val deferredClampTs: Long = 0L,
        /**
        ...
        /** Diagnose: Rueckgang dieses Zyklus [mg/dl]. */
        val evidenceDeclineMgdl: Double? = null,
    ...
    fun run(tempBasalFallback: Boolean, pumpe: FuseActivePump): Outcome {
        ...
        fun abort(
            ...
            evidenceDeclineMgdl = evidenz?.declineMgdl,
        ...
        val decisionVorEndpruefung: FuseController.Decision = run {
            ...
            // der Schwelle gestanden haette. Und auch dann kann eine Gefahr
            // zugleich anliegen, die Tor- und Ablehnungsreihenfolge verdecken -
            // deshalb getrennt erfasst. Rein beschreibend.
            val verlustUrsache = app.aaps.fuse.core.controller.EvidenceLossDiagnosis.cause(
                reboundAusnahme.denial, evidenz?.phase, evidenz?.deductionMgdl,
                evidenz?.stockBeforeDeductionMgdl, evidenceConfig.stockFloorMgdl,
            )
            livenessEvidenceLossCause = verlustUrsache?.name
            val gefahren = app.aaps.fuse.core.controller.EvidenceLossDiagnosis.hazards(
                app.aaps.fuse.core.controller.EvidenceLossDiagnosis.Observation(
                    signalHealthy = step.health == Health.READY,
                    ledgerHold = ledgerView.hold,
                    measuredLow = measuredLow,
                    descentRisk = descentRisk.active,
                    descentRiskMarker = risk60?.active == true,
                    latchBlocksPositive = descentLatch.blocksPositive,
                    ukfRatePerMin = signal.ukfRatePerMin,
                    ukfFloorPerMin = LivenessChannel.UKF_FLOOR_MGDL_PER_MIN,
                    measuredVerdict = upfrontStabilitaet.verdict,
                    declineMgdl = evidenz?.declineMgdl,
                    turningDown = turnClassification.phase == TurnResponseShadow.Phase.TURNING_DOWN,
                ),
            )
        ...
        val decision: FuseController.Decision = run {
            ...
            if (latch.state.active) {
                ...
                // VARIANTE 1 - DER GRUND-WEG-AUSGANG (Default AUS).
                //
                // Er prueft, ob der AUSLOESER der Null noch besteht, statt
                // wie der Ruhe-Ausgang auf das vollstaendige Verschwinden
                // der Bolus-Ueberdeckung zu warten. Gemessen hat genau
                // diese Ueberdeckungs-Bedingung die Null nach dem Wegfall
                // des Grundes weitergehalten.
                //
                // ER GIBT KEIN INSULIN. Das Ende der Verriegelung stellt
                // hoechstens das Profilbasal wieder her; eine Rate darueber
                // existiert im ganzen Aktuationsraum nicht.
                val grundWeg = lowThreatResult.verdict == LowThreatGate.Verdict.NONE
                val erholungBestaetigt = step.health == Health.READY && !measuredLow &&
                    !descentRisk.active && signal.ukfRatePerMin.isFinite() &&
                    signal.ukfRatePerMin >= -0.03 && q1NichtFallend
                ...
                if (ruheAusgang || grundWegAusgang) {
                    ...
                    zeroLatchRuntime = DescentRecoveryLatch.Runtime()
                    // BEIDE Zaehler zurueck: der Latch ist weg, ein
                    // stehengebliebener Streak wuerde die naechste Phase
                    // sofort wieder oeffnen.
                    zeroCalmStreak = 0
                    zeroReasonGoneStreak = 0
                    // Der bisherige Ausgang behaelt seinen Namen, wenn er
                    // ohnehin getragen haette - sonst waere im Trail nicht
                    // zu sehen, ob die Variante ueberhaupt etwas geaendert
                    // hat.
        ...
        return Outcome(
            ...
            deferredOpenBeforeClampU = buchung.clampBeforeU,
            deferredClampHullAtClampU = buchung.clampHullU,
            deferredClampReductionU = buchung.clampReductionU,
            deferredClampReason = buchung.clampReason,
            deferredClampTs = buchung.clampTs,
     ...
     * den belasteten.
     */
    /** Reines Zwischenergebnis innerhalb von [buche]. */
    private data class Klemmung(
        val vorU: Double = 0.0,
        val huelleU: Double = 0.0,
        val wegU: Double = 0.0,
    ...
    private data class Buchung(
         ...
         * Runnerfeldern gehalten.
         *
         * Die erste Fassung merkte es sich prozessweit und setzte es nie
         * zurueck. Damit haette derselbe alte Clamp in JEDEM folgenden
         * Trailzyklus erneut gestanden - ohne Zeitstempel nicht als
         * Wiederholung erkennbar, und bei einer Summierung mehrfach
         * gezaehlt. Ein Ereignis gehoert dorthin, wo es entsteht.
         */
        val clampBeforeU: Double = 0.0,
        val clampHullU: Double = 0.0,
        val clampReductionU: Double = 0.0,
