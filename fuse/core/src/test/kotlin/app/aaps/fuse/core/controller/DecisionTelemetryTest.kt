...
class DecisionTelemetryTest {
    ...
    fun `die Kappenliste enthaelt jede geprueften Grenze`() {
        val d = FuseController.decide(state(), flat(), evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `der erschoepfte IOB-Fall berichtet beide Grenzen`() {
        val d = FuseController.decide(state(iobTh = 1.0, maxIob = 1.0, netIob = 2.0), flat(), evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `eine Bremse die nur den Guard senkt setzt nur das Guard-Bit`() {
        ...
        val d = FuseController.decide(state(), main, restraint = brake, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `das Bodendefizit passt zur entschiedenen Bahn`() {
        ...
        val d = FuseController.decide(state(), p, limits, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `eine sichere Bahn meldet kein Defizit und keine Zeit`() {
        val d = FuseController.decide(state(), flat(), FuseController.Limits(guardFloorMgdl = 80.0), evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `auch die Bremsbahn kann den Boden reissen`() {
        ...
        val d = FuseController.decide(state(), main, limits, restraint = brake, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
    ...
    fun `ohne Bremse folgt der kombinierte Index der Hauptbahn`() {
        ...
        assertEquals(0, FuseController.decide(state(), steigend, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE).timeToMinCombinedMin)

        val fallend = pred(bg = 160.0, anchor = 150.0) { i -> if (i < 25) 140.0 else 110.0 }
        assertEquals(25, FuseController.decide(state(), fallend, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE).timeToMinCombinedMin)
    }
    ...
    fun `jeder Ausgang traegt dieselbe Telemetrie`() {
        ...
        val faelle = listOf(
            ...
            FuseController.Block.NO_DEMAND to FuseController.decide(
                ...
                restraint = pred(bg = 90.0, anchor = 160.0) { 100.0 }, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE
            ),
            FuseController.Block.MAX_IOB_REACHED to FuseController.decide(
                state(iobTh = 1.0, maxIob = 1.0, netIob = 2.0), main, limits, restraint = brake, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE
            ),
            FuseController.Block.IOB_TH_REACHED to FuseController.decide(
                state(iobTh = 1.0, maxIob = 8.0, netIob = 2.0), main, limits, restraint = brake, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE
            ),
            FuseController.Block.NONE to FuseController.decide(state(), main, limits, restraint = brake, evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE),
        )
    ...
    fun `die Kandidatenstufe ersetzt die Kappenliste der Basis`() {
        val base = FuseController.decide(state(), flat(bg = 300.0), evidenceCreditActive = false, evidenceMayOverrideRebound = false, lowThreat = LowThreatGate.Verdict.NONE)
